...
운영 중인 Next.js 애플리케이션에서 정적 리소스를 CDN으로 옮겨 서버 부하를 덜고, 클라이언트측 누수 후보들도 하나씩 정리했다. 그래도 운영 환경에서 메모리는 여전히 꾸준히 우상향하고 있었다. 이번 글에서는 **재현 환경을 만들고**, 그 안에서 실제 원인 한 점을 찾아낸 과정을 정리한다.
...
그 결과 한 컴포넌트가 들어갈 때만 에러가 재현됐다. 바로 홈 화면 진입 시 노출되는 **메인 팝업 컴포넌트**였다.
...
앞서 전역 `QueryClient`의 `gcTime`을 30분 → 5분으로 줄여 두었고, 여기에 더해 컴포넌트 단위에서 길게 잡혀 있던 옵션도 제거했다. 두 변경의 합으로 부하 테스트와 운영 환경 모두에서 메모리가 안정적으로 회수되는 것을 확인했다.
...
캐시 옵션은 "성능을 위해 길게 잡는 것"이 아니라 **다음 GC 시점에 진짜 회수가 일어날 수 있는 길이**여야 한다.

다만 이 글을 쓰고 나서도 풀리지 않은 질문이 하나 남았다. "왜 클라이언트에서는 같은 옵션이 문제를 일으키지 않았을까?" 결과는 맞췄지만 이 글에서 설명한 메커니즘(컴포넌트 unmount → 카운트다운 → 트래픽으로 리셋)이 SSR 환경을 충분히 설명하지 못했기 때문이다. [다음 글 — gcTime 한 줄이 SSR을 죽인 진짜 이유](/posts/memory-leak-4-gctime-deep-dive)에서는 TanStack Query 소스 두 파일을 직접 열어 SSR 분기의 실제 동작을 따라가며, 이 글의 설명 일부를 정정한다.
