...
[1편](/articles/memory-leak-1-image-cdn)에서 정적 리소스를 CDN으로 옮겨 서버측 부하를 덜었다면, 이번 편에서는 클라이언트에서 직접 발견된 누수 케이스 네 가지를 정리한다.

## Chat Scroll 로직 개선

### 무엇이 문제였나

Nova 기능 단위 하위에 전역으로 적용되던 layout.tsx의 Chat Scroll 로직을 분석한 결과, 메모리 누수의 핵심 원인을 발견했다.
...
이 코드는 MutationObserver로 DOM 변경을 감지하고, 변경마다 setTimeout으로 스크롤을 최하단으로 이동시킨다. 메모리 누수의 원인은 다음 네 가지다.

1. Observer 연결 해제와 setTimeout 정리가 useEffect cleanup에서만 처리됨
2. layout.tsx에 구현되어 있어 Nova 영역을 **완전히 이탈해야만** cleanup 실행
3. Nova 내부 페이지 이동 시에는 cleanup이 실행되지 않음
4. setTimeout이 지속적으로 실행되며 scrollToBottom을 반복 호출

### 어떻게 고쳤나

layout에서 동작하던 로직을 chat 페이지 내부로 이동하고, 불필요한 setTimeout을 제거했다. 채팅 이탈 시 cleanup이 정상적으로 실행되어 GC가 수집할 수 있도록 변경했다.
...
기존 30분이던 gcTime을 default(5분)로 변경해 캐시 정리 주기를 단축했다.

정리 주기가 짧아지면 메모리 곡선의 모양 자체가 달라진다.

<MemoryGrowthDemo preset="gctime" />

**오른쪽처럼 오르내리는 톱니가 나와야 회수가 실제로 일어나고 있다는 뜻이다.**
...
1. `useScrollDirection`
   ...
   - 페이지 최상단 여부 (isAtTop)
   - 상단 고정 영역(50px) 내 여부 (isInTopLockArea)
2. `useHeaderStyle`
...
상태를 top / scroll-down / scroll-up 세 값의 단순 enum으로 정리하고, throttle로 핸들러 호출 빈도를 줄였다.
...
Timeline 분석에서는 InnerSlide가 리렌더링될 때마다 Heap이 튀는 현상이 확인됐다.
...
이는 **라이브러리 자체 이슈**로, 관련 GitHub 이슈에서도 동일한 현상이 보고되어 있다 — [react-slick #1257](https://github.com/akiran/react-slick/issues/1257). 자체 코드로는 회피가 불가능하다고 판단해, 슬라이더 자체를 다른 솔루션으로 교체하는 작업은 별도 트랙으로 분리해 진행했다.
...
하지만 **운영 환경의 서버 메모리는 여전히 우상향을 그리고 있었다.** 클라이언트에서 끊을 수 있는 누수는 다 끊었는데 서버 RSS가 계속 늘어난다면, 원인은 다른 층에 있다는 뜻이다. [3편 — 서버사이드 편](/articles/memory-leak-3-server-side)에서는 운영과 똑같은 OOM을 재현할 수 있는 부하 테스트 환경을 만들고, 그 안에서 진짜 원인 한 점을 좁혀 가는 과정을 다룬다.
