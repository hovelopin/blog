...
ESM은 tree-shaking에 필요한 정적 구조를 제공하지만 제거 결과까지 보장하지는 않는다. 배럴 파일에 부작용이 있거나 sideEffects 설정이 잘못되면 ESM이어도 사용하지 않는 코드가 제거되지 않는다. 이 내용은 2편에서 다룬다. 여기서 확인할 점은 하나다. **CJS만 배포하면 소비자 번들러가 내보내기 단위로 tree-shaking하기 어렵다.**
...
1. **상태를 갖지 않는다.** 순수 함수만 내보내면 모듈이 두 번 평가되어도 동작이 달라지지 않는다.
2. **상태를 단일 CJS 모듈에 두고**, ESM 파일은 그 모듈을 가져와 다시 내보내도록 구성한다. 두 형식이 같은 CJS 모듈의 상태를 사용하게 된다.
