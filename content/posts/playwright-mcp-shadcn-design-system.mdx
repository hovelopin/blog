...
> - Shadcn MCP로 만들 거고, 나만의 테마를 만들고 싶으니 globals.css를 적절히 수정/추가하면서 구현해줘.
...
실제 화면을 눈으로 확인하며 토큰을 맞춰가는 흐름이다.

<PipelineDemo preset="designSystem" />

마지막 단계에서 다시 열어보는 것이 핵심이다. 한 바퀴로 끝나는 일이 아니라 반복해서 좁혀간다.
