...
function draw(args: DrawArgs) {
  const { ctx, elapsed, palette } = args;
  const pad = 14;
  const gap = 20;
  ...
  const paintColumn = (
    ...
    colW: number,
    laneH: number,
    [title, frames, safe]: (typeof columns)[number],
  ) => {
    ...
    if (safe && blocked) {
      ...
      ctx.fillText("$$consumed === true → throw", x, laneH - 20);
      ctx.font = `10px ${MONO}`;
      ctx.fillStyle = palette.muted;
      ctx.fillText("2단계에서 사이클이 끊긴다", x, laneH - 8);
    } else if (!safe && frames >= MAX_FRAMES) {
      ctx.fillStyle = palette.danger;
      ctx.fillText("스택 한계까지 계속 쌓임 · CPU 100%", x, laneH - 20);
      ctx.font = `10px ${MONO}`;
      ctx.fillStyle = palette.muted;
      ctx.fillText("$$consumed = true 에 영영 도달하지 못한다", x, laneH - 8);
    } else if (!safe) {
      ctx.fillStyle = palette.muted;
      ctx.fillText("new Map(model) → 이터레이터가 재진입", x, laneH - 20);
    }
  };

  twoLanes(
    args,
    { pad, gap, divider: true },
    (x, w, laneH) => paintColumn(x, w, laneH, columns[0]),
    (x, w, laneH) => paintColumn(x, w, laneH, columns[1]),
  );
