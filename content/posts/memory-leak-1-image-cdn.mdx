...
이제 next.config의 `assetPrefix` 옵션을 사용하면 JS/CSS 파일들이 자동으로 CDN 경로로 치환된다.
...
assetPrefix 적용 후 빌드까지는 정상적으로 진행됐지만, **SCSS 파일 내에서 절대 경로로 작성된 이미지의 경우** assetPrefix 전체 경로가 아닌 origin(호스트)까지만 적용돼 /nova가 빠지는 문제가 있었다.

postcss.config에서 postcss-url로 SCSS의 /assets/ 경로를 /nova/assets/로 변환하는 후처리를 넣어 해결했다.
