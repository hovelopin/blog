...
피그마에 접속해 자신의 프로필을 클릭하면 Settings에 진입할 수 있다.

![Figma Settings 진입](/imports/claude-figma-mcp/image-002.png)

`Security → Generate new token`으로 API 키를 발급한다.

![API 토큰 발급](/imports/claude-figma-mcp/image-008.png)

## 2. Figma-MCP 서버 설치

MCP 프로토콜 연결이 가능한 AI 도구(Claude, Cursor, Windsurf 등)가 필요하다. 이 글에서는 Claude를 기준으로 진행한다.

먼저 Figma-MCP 서버를 설치한다. 서버는 [GLips/Figma-Context-MCP](https://github.com/GLips/Figma-Context-MCP) 레포지토리를 클론해서 실행한다.
...
`.env.sample` 파일에서 `FIGMA_API_KEY`에 1단계에서 발급받은 토큰을 입력한다.
...
파일명을 `.env.sample`에서 `.env`로 변경한 뒤 서버를 실행한다.

```bash
pnpm run dev
```

서버가 정상적으로 떠 있다면 콘솔에서 다음과 같은 로그를 확인할 수 있다.

![서버 실행 로그](/imports/claude-figma-mcp/image-003.png)

## 3. Claude Desktop과 연결

Figma-MCP를 Claude와 함께 쓰려면 Desktop 버전이 필요하다. [Claude Desktop Download](https://claude.ai/download)에서 OS에 맞는 버전을 받는다.

설치 후 실행해 설정 탭에 접속한다.

![Claude 설정 진입](/imports/claude-figma-mcp/image-006.png)

좌측 탭의 `개발자(Developer)`를 선택하고 설정 편집을 연다.

![개발자 탭](/imports/claude-figma-mcp/image-011.png)

`claude_desktop_config.json` 파일을 클릭한다.

![config 파일 열기](/imports/claude-figma-mcp/image-014.png)

아래 설정을 구성 파일에 추가한다.

> ⚠️ Claude를 설정했을 때 셋팅이 됐다 안 됐다 하는 문제가 발생하는 경우가 있다. 두 가지 설정을 모두 시도해 볼 수 있다.
...
설정이 완료되면 피그마에서 `Copy link to selection`으로 링크를 복사한 뒤, Claude에 링크와 함께 원하는 스타일링 도구(예: `styled-components`)와 프레임워크/라이브러리를 함께 전달한다.

![Figma 링크 복사](/imports/claude-figma-mcp/image-001.png)

Claude는 해당 링크를 분석해 다음과 같이 코드로 변환된 결과를 돌려준다.
...
생성된 결과물의 완성도가 꽤 높았다. 정의된 아이콘과 세부 CSS만 추가하면, UI 로직에 들이는 비용을 줄이고 핵심 비즈니스 로직에 더 집중할 수 있을 것 같다.

특히 Cursor처럼 IDE에 직접 연결되는 AI 도구를 쓰면 진행 중인 프로젝트 구조를 함께 이해하기 때문에 활용도가 더 높을 것으로 보인다.
