---
title: "i18next enableSelector는 런타임에서 어떻게 동작할까?"
...
author: "hovelopin"
cover: "/covers/i18next-selector-runtime-magnet-clean.png"
coverAlt: "알록달록한 문자 장난감 배경의 깨끗한 중앙 영역에 플라스틱 알파벳처럼 i18next가 놓인 커버 이미지"
...
1편에서는 문자열 key에 i18next 타입을 연결해 오타와 삭제된 key를 컴파일 단계에서 잡는 방법을 정리했다. 그 다음으로 마주치는 선택지가 `enableSelector`다. `enableSelector`를 설정하면 번역 key를 문자열로 직접 작성하지 않고 함수 형태로 표현할 수 있다.
...
그 이후의 번역 과정은 기존 i18next와 동일하다. 아래에서는 타입 선언이 아니라 런타임 동작을 따라간다. `useTranslation()`에서 받은 `t`가 어떻게 만들어지고, selector 함수가 어떤 과정을 거쳐 문자열 키가 되는지 내부 구현 흐름으로 확인해보자.

---

## 전체 흐름 먼저 보기

예제 하나를 끝까지 따라가 보자. 세부 구현으로 들어가기 전에, 함수가 어느 지점을 거쳐
번역 엔진에 닿는지부터 훑어두면 뒤 내용이 훨씬 수월하다.

<PipelineDemo preset="selectorRuntime" />
...
핵심은 중간에 selector를 문자열 키로 변환하는 단계가 하나 추가되었다는 점이다. selector → 문자열 key 변환이 어디서 시작되는지 보려면 먼저 `t`가 어떤 함수인지부터 봐야 한다.
...
중요한 점은 **이 시점에서 화살표 함수가 아직 실행되지 않는다**는 데 있다. `t()`가 받는 값은 다음과 같다.
...
이때부터 `$`가 실제 값을 갖는다. 이제 selector를 호출하는 `t`가 어디서 만들어지는지 살펴보자.
...
`useTranslation()`이 돌려주는 `t`는 단순히 전역 `i18next.t`를 그대로 반환한 것이 아니다. `useTranslation()`은 현재 컴포넌트의 상태에 맞는 **전용 `t` 함수**를 만들어 돌려준다.

### useTranslation()에서 하는 일

[`react-i18next` v16.6.1의 `useTranslation.js`](https://github.com/i18next/react-i18next/blob/v16.6.1/src/useTranslation.js)에서는 대략 다음 과정을 거친다.
...
중요한 지점은 [`getFixedT()`](https://github.com/i18next/i18next/blob/v26.4.0/src/i18next.js#L420)다.
...
위 로직을 실행하면 내부적으로는 `common` namespace를 기본값으로 사용하는 `t`가 동작하며, selector 함수도 바로 전용 `t` 안으로 들어온다.
...
이제 [`getFixedT()`](https://github.com/i18next/i18next/blob/v26.4.0/src/i18next.js#L420)가 만든 전용 `t` 안으로 들어가 보자. [원본 구현](https://github.com/i18next/i18next/blob/v26.4.0/src/i18next.js#L422-L462)을 단순화하면 대략 이런 형태다.
...
따라서 `typeof key === 'function'` 분기에서 [`keysFromSelector`](https://github.com/i18next/i18next/blob/v26.4.0/src/selector.js#L18)가 호출된다.

```tsx
keysFromSelector(($) => $.pages.notFound.title);
```

[`keysFromSelector`](https://github.com/i18next/i18next/blob/v26.4.0/src/selector.js#L18) 안에서 selector가 실행되고 문자열 키로 변환된다. 다음 단계의 핵심은 Proxy다.

---

## Proxy를 먼저 이해하자

i18next 구현으로 들어가기 전에 Proxy를 먼저 짚고 가자. Proxy는 원본 객체에 바로 접근하지 않고, 앞에 가로채는 객체를 세우는 JavaScript 기능이다. 원본 객체를 `target`, 앞에서 접근을 가로채는 규칙을 `handler`라고 부른다.

```ts
const proxy = new Proxy(target, handler);
```

`handler`에 들어가는 함수는 **trap**이라고 부른다. 값 읽기, 값 쓰기, `in` 연산자, 함수 호출 같은 동작을 가로챌 수 있다. 그중 가장 자주 보는 trap은 `get`과 `set`이다.
...
`safeUser.age`를 읽으면 원본 객체에는 `age`가 없지만 바로 `undefined`가 나오지 않는다. 먼저 `get` trap이 실행되고, handler가 정한 규칙에 따라 `"정보 없음"`이 반환된다. 원본 객체를 고치지 않고 읽기 동작만 바꾼 셈이다.
...
`person.age = 20`을 실행하면 `set` trap에는 `target`, `key`, `value`가 차례대로 들어온다. 위 예시에서는 `key`가 `"age"`이고 `value`가 `20`이다. 값이 숫자이면 원본 객체에 저장하고 `true`를 반환한다. `set` trap은 쓰기 성공 여부를 불리언으로 알려줘야 한다.

Proxy가 유용한 이유는 원본 객체 코드를 바꾸지 않고도 접근 방식을 바꿀 수 있기 때문이다. i18next selector도 같은 성질을 이용한다. `$`가 실제 번역 리소스가 아니어도, `$.pages.notFound.title`처럼 접근하는 순간을 Proxy가 가로채 경로를 기록할 수 있다.

하나 더 알아둘 API가 있다. `Proxy.revocable`이다.

```ts
const { proxy, revoke } = Proxy.revocable(target, handler);
```

일반 Proxy는 한 번 만들면 계속 접근할 수 있다. 반면 `Proxy.revocable`은 `proxy`와 함께 `revoke` 함수를 반환한다. `revoke()`를 호출하면 Proxy를 통하는 통로가 끊긴다.
...
취소된 Proxy는 읽기, 쓰기, `in` 연산자 같은 접근에서 모두 `TypeError`를 낸다. `revoke()`를 여러 번 호출해도 추가 에러는 나지 않는다. 끊긴 것은 Proxy를 통하는 통로일 뿐이라 원본 객체는 그대로 남는다.
...
[`selector.js`](https://github.com/i18next/i18next/blob/v26.4.0/src/selector.js)의 핵심은 Proxy다. selector가 실제 리소스 객체를 받는 것은 아니지만, Proxy를 받으면 프로퍼티 접근을 모두 기록할 수 있다. `createProxy` 구현을 단순화하면 다음과 같다.
...
중요한 점은 **selector의 반환값이 문자열이 아니라는 것**이다.
...
경로 오염은 조용히 발생하기 때문에 오히려 위험하다. i18next가 이전 Proxy를 즉시 폐기하는 이유도 `createProxy`의 `revoke` 호출에 있다.
...
관련 코드는 [selector.js의 namespace 판정 로직](https://github.com/i18next/i18next/blob/v26.4.0/src/selector.js#L36-L54)과 대략 같다.
...
여기까지 오면 selector와 관련된 특별한 동작은 거의 끝난다. `fixedT`가 `this.t()`를 호출하면 결국 [`Translator.translate`](https://github.com/i18next/i18next/blob/v26.4.0/src/Translator.js#L100)로 이어진다. 최종적으로는 다음 호출과 동일해진다.
...
| 단계 | 위치                                                                                            | 역할                               |
| ---- | ----------------------------------------------------------------------------------------------- | ---------------------------------- |
| 1    | [`useTranslation`](https://github.com/i18next/react-i18next/blob/v16.6.1/src/useTranslation.js) | i18n 상태와 namespace를 준비       |
| 2    | [`getFixedT`](https://github.com/i18next/i18next/blob/v26.4.0/src/i18next.js#L420)              | 현재 컴포넌트에 맞는 전용 `t` 생성 |
| 3    | [`fixedT`](https://github.com/i18next/i18next/blob/v26.4.0/src/i18next.js#L422)                 | selector 함수인지 확인             |
| 4    | [`keysFromSelector`](https://github.com/i18next/i18next/blob/v26.4.0/src/selector.js#L18)       | Proxy를 selector에 전달            |
| 5    | [`createProxy`](https://github.com/i18next/i18next/blob/v26.4.0/src/selector.js#L3)             | 프로퍼티 접근 경로 기록            |
| 6    | [`PATH_KEY`](https://github.com/i18next/i18next/blob/v26.4.0/src/selector.js#L1)                | 기록된 경로 회수                   |
| 7    | [namespace 판정](https://github.com/i18next/i18next/blob/v26.4.0/src/selector.js#L36-L54)       | 배열을 문자열 키로 변환            |
| 8    | [`Translator`](https://github.com/i18next/i18next/blob/v26.4.0/src/Translator.js#L100)          | 기존 i18next 방식으로 번역 조회    |
...
`t(($) => $.pages.notFound.title)`는 번역 엔진을 갈아엎은 API가 아니다. selector 함수를 문자열 key로 바꾸는 레이어가 앞에 하나 더 붙었을 뿐이다.

런타임에서 추가되는 것은 Proxy·revoke·Symbol 세 가지다. 경로를 기록하고, 오염을 막고, 기록된 배열을 꺼낸다. 이 변환이 끝나면 흐름은 다시 `t('pages.notFound.title')`와 같아진다.
...
- 1편: [i18next 문자열 key에 타입을 붙이는 과정](/articles/i18next-typescript-setup)
- 3편: [문자열 key vs selector: i18next 타입은 어떻게 만들어질까](/articles/i18next-key-vs-selector-types)
