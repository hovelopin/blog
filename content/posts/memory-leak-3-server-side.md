...
다만 이 글을 쓰고 나서도 풀리지 않은 질문이 하나 남았다. "왜 클라이언트에서는 같은 옵션이 문제를 일으키지 않았을까?" 결과는 맞췄지만 이 글에서 설명한 메커니즘(컴포넌트 unmount → 카운트다운 → 트래픽으로 리셋)이 SSR 환경을 충분히 설명하지 못했기 때문이다. [4편 — gcTime 한 줄이 SSR을 죽인 진짜 이유](/posts/memory-leak-4-gctime-deep-dive)에서는 TanStack Query 소스 두 파일을 직접 열어 SSR 분기의 실제 동작을 따라가며, 이 편의 설명 일부를 정정한다.
