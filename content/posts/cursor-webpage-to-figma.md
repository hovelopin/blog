...
셋업이 끝나면 Cursor의 `Settings → MCP Servers`에서 연결된 서버를 확인할 수 있다.
...
하단 액션 탭을 클릭한 뒤 `Import from manifest`를 선택한다.

![Import from manifest](/imports/cursor-webpage-to-figma/image-008.png)

위에서 클론받은 프로젝트의 `src/cursor_mcp_plugin/manifest.json`을 선택한다.
...
연결이 정상적으로 됐다면 **터미널에 표시된 Channel ID**와 **Figma 플러그인에 표시된 Channel ID**가 같다. 두 값이 일치하면 MCP 서버와 Figma 플러그인이 연결된 상태다.
...
변환된 결과물이 원본과 완전히 일치하지는 않는다. 하지만 **디자이너가 없는 팀 환경**에서는 충분히 실용적이다.
...
특히 **빠른 프로토타이핑**이나 **PoC** 단계에서 디자인 시안이 필요할 때, 디자이너를 기다리지 않고 즉시 작업에 들어갈 수 있다는 점이 가장 큰 장점이다. 완벽한 디자인 복제보다는 **빠른 기획·커뮤니케이션 도구**로 접근하면 팀 생산성을 끌어올릴 수 있다.
